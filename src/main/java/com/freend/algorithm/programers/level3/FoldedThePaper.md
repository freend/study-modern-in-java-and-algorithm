...
- 이것 또한 규칙성이 있는건 알았는데 정확한 규칙성을 찾지 못해서 하루를 허비했다.
- 규칙은 아래의 참고 사이트에서 가져왔다.
- 1번 접으면 0이 된다.
- 2번 접으면 위의 0을 기준으로 0 -> 1 -> 0이 반복되는 구조다. 라는 설명을 듣고 다음과 같이 했다.
  - 두개의 queue를 만든다. 변경 전 큐 변경 후 큐
  - 종이 접는 횟수만큼 for문을 반복한다.
  - 변경후 큐를 변경전 큐로 만든다. 그리고 시작번호를 0으로 만든다.
  - 변경 전 큐에서 하나씩 뽑고 뒤에 1을 넣는다 그리고 번호를 0으로 만들고..
- 이것을 계속 반복한 후 최종 배열을 반환하면 된다.



- [참고사이트](https://sewonkimm.github.io/algorithm/2019/12/04/paper.html)