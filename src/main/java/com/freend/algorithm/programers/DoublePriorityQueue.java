...
public class DoublePriorityQueue {
     ...
     * D 1	큐에서 최댓값을 삭제합니다.
     * D -1	큐에서 최솟값을 삭제합니다.
     * 이중 우선순위 큐가 할 연산 operations가 매개변수로 주어질 때, 모든 연산을 처리한 후 "큐가 비어있으면 [0,0]" 비어있지 않으면 [최댓값, 최솟값]을
    ...
    public int[] solution(String[] operations) {
        ...
        for (String operation : operations) {
            order = operation.split(" ");
            switch (order[0].toUpperCase()) {
                case "I":
                    addOrder(order[1]);
                    break;
                case "D":
        ...
        answer[0] = operationList.get(operationList.size() -1);
        answer[1] = operationList.get(0);
        return answer;
    }
    ...
    private void addOrder(String s) {
        operationList.add(Integer.parseInt(s));
